      ...
      <DialogContent showCloseButton={false} className="max-h-[90vh] max-w-5xl rounded-2xl p-6">
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex items-center gap-2">
            <Select value={pageSize} onValueChange={(value) => setPageSize(value as (typeof PAGE_SIZE_OPTIONS)[number])}>
              <SelectTrigger className="h-10 w-[120px] rounded-xl border-white/[0.1] bg-white/[0.06] text-white/70">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        <div className="rounded-xl border border-white/[0.08]">
          ...
          <div className="max-h-[420px] overflow-auto">
            {pagedFiles.length === 0 ? (
              <div className="flex items-center justify-center py-12 text-sm text-white/25">没有匹配的远程账号</div>
            ) : (
              <div className="divide-y divide-white/[0.05]">
                {pagedFiles.map((item) => (
                  <label key={item.name} className="flex cursor-pointer items-center gap-3 px-4 py-3 hover:bg-white/[0.04]">
                    <Checkbox
                      checked={selectedNames.includes(item.name)}
                      onCheckedChange={(checked) => toggleFile(item.name, Boolean(checked))}
                    />
                    <div className="min-w-0">
                      <div className="truncate text-sm font-medium text-white/70">{item.email || item.name}</div>
                      <div className="truncate text-xs text-white/30">{item.name}</div>
                    </div>
                  </label>
                ))}
              </div>
