        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-white/40">先配置连接，再按需查询远程账号并选择导入到本地号池。</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {pools.length > 0 ? <Badge className="rounded-md px-2.5 py-1">{pools.length} 个连接</Badge> : null}
            <Button className="h-9 rounded-xl bg-white px-4 text-stone-950 hover:bg-white/90" onClick={openAddDialog}>
        ...
        {isLoadingPools ? (
          ...
          <div className="flex flex-col items-center justify-center gap-3 rounded-xl bg-white/[0.04] px-6 py-10 text-center">
            ...
            <div className="space-y-1">
              <p className="text-sm font-medium text-white/60">暂无 CPA 连接</p>
              <p className="text-sm text-white/35">点击「添加连接」保存你的 CLIProxyAPI 信息。</p>
            </div>
          </div>
        ) : (
          <div className="space-y-3">
            {pools.map((pool) => {
              ...
              return (
                <div key={pool.id} className="flex flex-col gap-3 rounded-xl border border-white/[0.06] bg-white/[0.04] px-4 py-3">
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <div className="text-sm font-medium text-white/90">{pool.name || pool.base_url}</div>
                      <div className="truncate text-xs text-white/35">{pool.base_url}</div>
                    </div>
