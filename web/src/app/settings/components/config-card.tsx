      ...
      <CardContent className="space-y-4 p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2">
            <label className="text-sm text-white/60">账号刷新间隔</label>
            <Input
            ...
            />
            <p className="text-xs text-white/35">单位分钟，控制账号自动刷新频率。</p>
          </div>
          <div className="space-y-2">
            <label className="text-sm text-white/60">全局代理</label>
            <Input
              ...
              className="h-10 rounded-xl"
            />
            <p className="text-xs text-white/35">留空表示不使用代理。</p>
            {proxyTestResult ? (
              <div
          ...
          <div className="space-y-2">
            <label className="text-sm text-white/60">图片访问地址</label>
            <Input
              ...
              className="h-10 rounded-xl"
            />
            <p className="text-xs text-white/35">用于生成图片结果的访问前缀地址。</p>
          </div>
          <div className="space-y-2">
            <label className="text-sm text-white/60">图片自动清理</label>
            <Input
              ...
              className="h-10 rounded-xl"
            />
            <p className="text-xs text-white/35">自动删除多少天前的本地图片。</p>
          </div>
          <div className="space-y-2">
            <label className="text-sm text-white/60">图片轮询超时</label>
            <Input
              ...
              className="h-10 rounded-xl"
            />
            <p className="text-xs text-white/35">单位秒，等待上游图片结果的最长时间。</p>
          </div>
          <div className="space-y-2">
            <label className="text-sm text-white/60">单账号图片并发</label>
            <Input
          ...
          <div className="space-y-3 rounded-xl border border-white/[0.06] bg-white/[0.04] px-4 py-3">
            <div>
              <label className="text-sm text-white/60">控制台日志级别</label>
              <p className="mt-1 text-xs text-white/35">不选择时使用默认 info / warning / error。</p>
            </div>
            <div className="grid grid-cols-2 gap-2">
              {logLevelOptions.map((level) => (
                <label key={level} className="flex items-center gap-2 text-sm capitalize text-white/60">
                  <Checkbox
          ...
          <div className="space-y-2 md:col-span-2">
            <label className="text-sm text-white/60">全局附加指令</label>
            <Textarea
              ...
              className="min-h-28 rounded-xl font-mono text-xs shadow-none"
            />
            <p className="text-xs text-white/35">每次请求都会作为 system 消息注入，可用于审核用户提示词、避免违规内容、统一约束模型行为或固定角色设定。</p>
          </div>
          <div className="space-y-2 md:col-span-2">
            <label className="text-sm text-white/60">敏感词</label>
            <Textarea
              ...
              className="min-h-28 rounded-xl font-mono text-xs shadow-none"
            />
            <p className="text-xs text-white/35">只要用户请求包含任意敏感词，就直接返回拒绝。</p>
          </div>
          <div className="space-y-4 rounded-xl border border-white/[0.06] bg-white/[0.04] px-4 py-3 md:col-span-2">
            ...
            <div className="rounded-lg border border-white/[0.06] bg-white/[0.04] px-3 py-2 text-xs text-white/55">
              当前待保存模式：
              ...
              <span className="ml-2 text-white/35">修改后需要点保存，或通过测试/同步按钮自动保存。</span>
            </div>
            <div className="grid gap-4 md:grid-cols-3">
              <div className="space-y-2">
                <label className="text-sm text-white/60">保存模式</label>
                <Select
                ...
                >
                  <SelectTrigger className="h-10 rounded-xl shadow-none">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-2 md:col-span-2">
                <label className="text-sm text-white/60">WebDAV URL</label>
                <Input
              ...
              <div className="space-y-2">
                <label className="text-sm text-white/60">用户名</label>
                <Input
              ...
              <div className="space-y-2">
                <label className="text-sm text-white/60">远端目录</label>
                <Input
              ...
              <div className="space-y-2 md:col-span-3">
                <label className="text-sm text-white/60">公开访问前缀</label>
                <Input
                  ...
                  disabled={!config?.image_storage?.enabled}
                />
                <p className="text-xs text-white/35">留空时返回本应用 /images/... 代理地址；填入后直接返回公开图片地址。</p>
              </div>
            </div>
          </div>
          <div className="space-y-4 rounded-xl border border-white/[0.06] bg-white/[0.04] px-4 py-3 md:col-span-2">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <div className="space-y-2">
                <label className="text-sm text-white/60">Base URL</label>
                <Input value={String(config?.ai_review?.base_url || "")} onChange={(event) => setAIReviewField("base_url", event.target.value)} placeholder="https://api.openai.com" className="h-10 rounded-xl" />
              </div>
              <div className="space-y-2">
                <label className="text-sm text-white/60">API Key</label>
                <Input value={String(config?.ai_review?.api_key || "")} onChange={(event) => setAIReviewField("api_key", event.target.value)} placeholder="sk-..." className="h-10 rounded-xl" />
              </div>
              <div className="space-y-2">
                <label className="text-sm text-white/60">Model</label>
                <Input value={String(config?.ai_review?.model || "")} onChange={(event) => setAIReviewField("model", event.target.value)} placeholder="gpt-5.4-mini" className="h-10 rounded-xl" />
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-sm text-white/60">审核提示词</label>
              <Textarea value={String(config?.ai_review?.prompt || "")} onChange={(event) => setAIReviewField("prompt", event.target.value)} placeholder="判断用户请求是否允许。只回答 ALLOW 或 REJECT。" className="min-h-24 rounded-xl text-xs shadow-none" />
            </div>
          </div>
        </div>
