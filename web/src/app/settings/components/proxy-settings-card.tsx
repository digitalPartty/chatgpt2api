        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-center gap-3">
            <div className="flex size-10 items-center justify-center rounded-xl bg-white/[0.08]">
              <Link2 className="size-5 text-white/60" />
            </div>
            <div>
              <h2 className="text-lg font-semibold tracking-tight">全局代理</h2>
              <p className="text-sm text-white/40">为系统中的出站请求配置统一代理，保存后会立即生效。</p>
            </div>
          </div>
        ...
        {isLoadingConfig ? (
          ...
          <>
            <div className="space-y-2">
              <label className="text-sm font-medium text-white/60">代理地址</label>
              <Input
                value={proxy}
                onChange={(event) => {
